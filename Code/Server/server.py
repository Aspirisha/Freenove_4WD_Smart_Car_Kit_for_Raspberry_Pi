...
class Server:
    ...
    def StartTcpServer(self):
        ...
        logger.info("Server address: %s", HOST)
    ...
    def StopTcpServer(self):
        ...
        try:
            ...
            logger.info("No client connection: %s", e)
    ...
    def readdata(self):
        try:
            ...
            while True:
                try:
                    logger.info("Waiting for command data from client...")
                    ...
                    if self.tcp_Flag:
                        self.Reset()
                    break
                logger.info("Received data from client: %s", AllData)
                if len(AllData) < 5:
                    restCmd = AllData
                    if restCmd == "" and self.tcp_Flag:
                        logger.info("Resetting server due to empty data")
                        self.Reset()
                        break
                restCmd = ""
                if AllData == "":
                ...
                for oneCmd in cmdArray:
                    ...
                    if data == None:
                        ...
                        logger.info(
                            "Processing mode command: mode is set to %s", data[1]
                        )
                        ...
                        logger.info(
                            "Processing motor command: mode is set to %s", self.Mode
                        )
                        try:
                            data1 = int(data[1])
                            data2 = int(data[2])
                        ...
                        logger.info(
                            "Processing m_motor command: mode is set to %s", self.Mode
                        )
                        try:
                            data1 = int(data[1])
                            data2 = int(data[2])
                        ...
                        logger.info(
                            "Processing car rotate command: mode is set to %s",
                            self.Mode,
                        )
                        try:
                            ...
                            if data4 == 0:
                                ...
                                Rotate_Mode = Thread(
                                    target=self.PWM.Rotate, args=(data3,)
                                )
                                Rotate_Mode.start()
                        except:
                            pass
                        ...
                        logger.info("Processing led command")
                        try:
                            data1 = int(data[1])
                            data2 = int(data[2])
                            data3 = int(data[3])
                        ...
                        logger.info("Processing led mod command")
                        self.LedMoD = data[1]
                        try:
                        ...
                        logger.info("Processing buzzer command")
                        try:
                            self.buzzer.run(data[1])
                        except:
                            pass
                    elif cmd.CMD_LIGHT in data:
                        logger.info("Processing light command")
                        if data[1] == "1":
                            self.Light = True
                            self.lightTimer = threading.Timer(0.3, self.sendLight)
                            self.lightTimer.start()
                        ...
                        try:
                            self.send(
                                cmd.CMD_POWER + "#" + str(round(ADC_Power, 2)) + "\n"
                            )
                        except:
                            pass
        except Exception as e:
            logger.error("Error in readdata thread: %s", e)
    ...
    def sendLight(self):
        if self.Light == True:
            ...
            try:
                self.send(
                    "CMD_MODE#1" + "#" + str(ADC_Light1) + "#" + str(ADC_Light2) + "\n"
                )
            except:
                self.Light = False
    ...
    def sendLine(self):
        if self.Line == True:
            ...
            try:
                self.send(
                    "CMD_MODE#2" + "#" + str(Line1) + str(Line2) + str(Line3) + "\n"
                )
            except:
                self.Line = False
